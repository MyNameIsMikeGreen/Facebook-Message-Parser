...
# Run with "python [script_filename] [Facebook_messages_archive] [alias_filename] [line_number_of_preferred_alias]"
...
def strip_time_zone(string):
    if string[-3:] == "UTC": return string[:-4]
    return string[:-7]
...
for block in contents:
    if block.tag == "div":  # All threads are held in some empty container div
        threads = all_elements_with_tag_and_attributes(block, "div", "class", "thread")  # All the threads in this div
        for thread in threads:
            ...
            if len(thread_names_without_user) != 1: continue    # Group chats not supported
            ...
            for i in range(0, len(thread), 2):
                ...
                message_time_raw_string = first_element_with_tag_and_attributes(header, "span", "class", "meta").text
                ...
                try:
                    ...
                    run_query(insertion_query)
                except sqlite3.OperationalError:
                    print "ERROR: ", str(insertion_query)
                    continue

                print "Sender: " + str(message_sender) + ", Time: " + str(message_time) + ", Message: " + str(message_body)


#####
##### ALL DATA INSERTED TO DATABASE AT THIS POINT - RUN ALL CUSTOM QUERIES BELOW. #####
#####


# Example query. Output all messages to and from user account in order of sending.
all_messages_sorted_query = run_query("SELECT Message_Sender, Message_DateTime, Message_Text FROM Messages ORDER BY datetime(Message_DateTime)")
